...
```{r, include = F}
# testing 
#params <- list(taxonkey= 5333412, out_file= "untitled.tif", n_bootraps= 6)
```
...
#SDM
library(flexsdm) #statistical
library(spThin) #spatial thinning
library(rgbif) #getting gbif data
...
```{r}
source("R/get_gbif_data.R")
source("R/get_elter_data.R")
...
```{r load_gbif_data}
gbif_data <- get_gbif_data(params$taxonkey)

sci_name <- gbif_data$scientificName %>% first() %>% strsplit(" ")
sci_name <- paste(sci_name[[1]][c(1,2)],collapse = " ")
```

eLTER

```{r load_elter_data}
elter_data <- get_elter_data(sci_name)
```

Compile full dataset

```{r}
species_data <- bind_rows(elter_data,gbif_data)
```
...
points(species_data[,c("x", "y")], col = "#00000480")
...
```{r occ_filtering}
species_data$id <- 1:nrow(species_data) # adding unique id to each row
species_data_f <- species_data %>%
  occfilt_env(
    data = .,
    x = "x",
    y = "y",
  ...
  left_join(species_data, by = c("id", "x", "y"))

plot(layer1)
plot(crop(ca, layer1), add=TRUE)
points(species_data[,c("x", "y")], col = "#00000480")
points(species_data_f[,c("x", "y")], col = "#ff0000")
```
...
```{r partitioning}
set.seed(10)
species_data_f_partitioned <- species_data_f %>%
  part_random(
    data = .,
    pr_ab = "pr_ab",
...
bg <- species_data_f_partitioned %>% 
  split(f=species_data_f_partitioned$.part) %>%
...
species_data_pa <- bind_rows(species_data_f_partitioned, bg)

plot(layer1)
plot(crop(ca, layer1), add=TRUE)
points(species_data_f[,c("x", "y")], col = "#5DC86180")
points(bg[,c("x", "y")], col = "red")

#extract data from raster
species_data_pa <- species_data_pa %>% sdm_extract(
    data = .,
    x = "x",
    y = "y",
    env_layer = env_data,
...
species_data_pa %>% select(names(env_data))%>% is.na() %>%colSums()
```
...
variability <- rast(individ_models) %>% diff() %>% sum() %>% abs()
plot(variability)
...
values(distance_rast)[cellFromXY(distance_rast,xy=as.matrix(species_data[,c("x","y")]))] <- -1
