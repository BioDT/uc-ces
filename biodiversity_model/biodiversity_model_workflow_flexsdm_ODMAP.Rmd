...
```{r}
# obtain GBIF username, password, and email credentials
...
while(status != "SUCCEEDED") {
  Sys.sleep(10) # Wait for 20 seconds before checking again
  ...
  occ_download_get(res, path = "GBIF_download", full.names = TRUE, overwrite = TRUE)
  
  # Unzip and read the data
  unzip(list.files("GBIF_download", full.names = TRUE)[1], exdir = "GBIF_data")

gbif_data = read.delim("GBIF_data/occurrence.txt", header = TRUE)

# Your date-time string
date_string <- "2024-03-01T12:57:18.394+00:00"

# Convert to POSIXct, which handles the ISO 8601 format natively
date_time <- as.POSIXct(date_string, format="%Y-%m-%dT%H:%M:%OS", tz = "UTC")
...
input_list$d_bio_4 = paste("data obtained from GBIF API with DOI: ", doi, " at datetime: " access_datetime)
...
input_list$p_uncertainty_2 <- "The models are trained using GBIF datasets. There may be biases introduced in the methods of data collection and source contributor."
